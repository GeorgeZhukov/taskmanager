...
!function(a, b) {
    ...
    d.directive("input", ["$compile", function(b) {
    ...
    ]), d.directive("djngError", function() {
        return {
            restrict: "A",
            require: "?^form",
            link: function(b, c, d, e) {
                var f;
                e&&!a.isUndefined(d.name) && "bound-field" === d.djngError && (f = e[d.name], f.$setValidity("bound", !1), f.$parsers.push(function() {
                    f.$setValidity("bound", !0), c.removeAttr("djng-error")
                }))
            }
        }
    ...
    }), d.directive("validateMultipleFields", function() {
        return {
            restrict: "A",
            require: "^?form",
            scope: !0,
            compile: function(b) {
                return a.forEach(b.find("input"), function(b) {
                    b = a.element(b), b.attr("ng-change", "changed()")
                }), {
                    post: function(b, c, d, e) {
                        function f(b) {
                            var c=!1;
                            a.forEach(i, function(a) {
                                c = c || a.$modelValue, a.clearRejected && a.clearRejected()
                            }), g.$setValidity("required", c), g.$setValidity("rejected", !0), g.$message = "", b && a.isString(h) && (g[h].$dirty=!0, g[h].$pristine=!1)
                        }
                        var g, h, i = [];
                        if (b.changed = function() {
                            f(!0)
                        }, e) {
                            g = e;
                            try {
                                h = a.fromJson(d.validateMultipleFields)
                            } catch (j) {
                                h = d.validateMultipleFields
                            }
                            a.forEach(c.find("input"), function(a) {
                                h.indexOf(a.name) >= 0 && i.push(g[a.name])
                            }), f()
                        }
                    }
                }
        ...
        function c(a) {
            var b = a.$viewChangeListeners.push(a.clearRejected = function() {
                a.$message = "", a.$setValidity("rejected", !0), a.$viewChangeListeners.splice(b - 1, 1), delete a.clearRejected
            })
        }
        function d(b) {
            return a.isArray(b.$viewChangeListeners)
        }
        var e = "__all__";
        return {
            setErrors: function(f, g) {
                if (f.$message = "", f.$error.hasOwnProperty("rejected") && a.isArray(f.$error.rejected)) {
                    var h = f.$error.rejected.concat();
                    a.forEach(h, function(b) {
                        var c, e = b.$name;
                        f.hasOwnProperty(e) && (c = f[e], d(c) && c.clearRejected ? c.clearRejected() : (c.$message = "", a.forEach(c, function(a) {
                            a && d(a) && a.clearRejected && a.clearRejected()
                        })))
                    })
                }
                return a.forEach(g, function(b, g) {
                    var h;
                    b.length > 0 && (g === e ? (f.$message = b[0], f.$setPristine()) : f.hasOwnProperty(g) && (h = f[g], h.$message = b[0], h.$setValidity("rejected", !1), h.$setPristine(), d(h) ? c(h) : a.forEach(h, function(a) {
                        a && d(a) && c(a)
                    })))
                }), b(g)
            }
    ...
    "use strict";
    var c = a.module("ng.django.rmi", []);
    c.provider("djangoRMI", function() {
        function c(d) {
            a.forEach(d, function(f, g) {
                ...
                f.hasOwnProperty("url") ? (f.headers["X-Requested-With"] = "XMLHttpRequest", d[g] = function(c) {
                    ...
                    if ("POST" === d.method) {
                        if (c === b)
                            throw new Error("Calling remote method " + g + " without data object");
                        d.data = c
                    } else 
                        "auto" === d.method && (c === b ? d.method = "GET" : (d.method = "POST", d.data = c));
                    return e(d)
                }) : c(f)
            })
        }
    ...
    "use strict";
    var c = a.module("ng.django.urls", []), d = "/angular/reverse/";
    c.service("djangoUrl", function() {
        function c(a, b, c) {
        ...
        function f(d, e) {
            if (!e)
                return d;
            var f = [];
            return c(e, function(c, d) {
                null != c && c != b && (a.isObject(c) && (c = a.toJson(c)), f.push(encodeURIComponent(d) + "=" + encodeURIComponent(c)))
            }), d + ( - 1 == d.indexOf("?") ? "?" : "&") + f.join("&")
        }
        this.reverse = function(b, e) {
            var g = f(d, {
                djng_url_name: b
            });
            if (Array.isArray(e))
            ...
            var h = {};
            return c(e, function(a, b) {
                h["djng_url_kwarg_" + b] = a
            }), a.equals(h, {}) ? g : f(g, h)
        }
    })
